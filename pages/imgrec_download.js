...
function DownloadFilePage({ user }) {
  ...
  const handleDownload = async (req) => {
    ...
    try {
      const response = await fetch(process.env.NEXT_PUBLIC_BASE_URL +`/img/image/result?fileName=${encoded_name}`, config);
      ...
      if (contentType.includes('application/json')) {
        ...
        const modifiedFileName = fileNameWithoutExtension.substring(fileNameWithoutExtension.indexOf('_') + 1);
  ...
  return (
    <Root className="root" style={{ display: "flex", flexDirection: "column" }}>
      ...
      <>
        <Progress active={progress < 100 ? 1 : 2} />
        <Typography className="content" variant="h6">
          {progress < 100 ? "Results are being generated" : "Results are Ready"}

          <Typography sx={{ marginTop: '5px' }}>{progress < 100 ? "The Download link will be sent to your email" : "The Download link has been sent to your email"}</Typography>
        </Typography>
        <Grid container justifyContent="center">
          <Grid item xs={12} sm={8}>
            <Box className="uploadBox">
              <Box style={{ backgroundColor: 'white', borderRadius: '10px', padding: '30px' }}>
                <Typography style={{ fontSize: '20px', color: 'black' }} variant="body1"><strong>{progress < 100 ? "Generating..." : "Download File"}</strong></Typography>
            ...
            <>
              {progress < 100 ? (
                ...
                <Link href="/imgrec_upload" style={{ display: 'flex', justifyContent: 'end', marginTop: '5px', textDecoration: 'none' }}>
                  <Button
                    style={{ borderRadius: '5px', marginLeft: '5px' }}
                    size="small"
                    variant="contained"
      ...
      <Snackbar
        ...
        onClose={handleClose}
      >
        <Alert severity="error" onClose={handleClose} style={{ whiteSpace: 'pre-line' }}>
